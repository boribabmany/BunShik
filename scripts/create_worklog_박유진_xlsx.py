...
DAILY = [
    ...
    ("2026-08-10", "신규 기능(옵션카드 슬라이드) 수동 테스트 체크리스트를 작성해 전 항목을 검증하고, 잘못 커밋되어 있던 kiosk-customer/build 산출물을 저장소에서 정리함."),
    ("2026-08-12", "결제 승인 확인(confirm) 재시도 시 이미 완료된 결제가 오류로 처리되던 문제를 수정하고, 저장소 전반의 줄바꿈 규칙을 통일했으며, 옵션카드 슬라이드(DragScrollRow) 컴포넌트에 대한 자동 테스트를 추가함."),
    ("2026-08-13", "고객용 키오스크 E2E(Playwright) 테스트 환경을 구축하고 매장식사·포장·옵션 메뉴·세트 메뉴 담기부터 결제 완료까지의 흐름을 검증했으며, 테스트 코드를 Page Object Model 구조(tests/pages/fixtures)로 분리함."),
    ("2026-08-19", "주문완료 화면에 실제 order_id 기반 영수증/주문번호표 출력 요청(POST /api/print-jobs) 연동을 추가함."),
    ("2026-08-20", "RTOS 연동에 맞춰 주문 상태 저장(useOrderStore)과 메뉴·결제·토스결제 성공·주문완료 화면의 실시간 처리를 보완함."),
    ("2026-08-25", "출력 작업(print_jobs) 등록\u2192RTOS 폴링\u2192완료 처리 전체 흐름을 검증하는 백엔드 통합 테스트(PrintJobFlowIntegrationTest)를 추가하고, 영문 홈 화면의 Dine In/Take Out 버튼 줄바꿈을 띄어쓰기로 수정함."),
]
...
AREAS = [
    ...
    ("품질 관리", "고객용 키오스크 자동 테스트 작성, Playwright 기반 E2E 테스트(매장식사·포장·옵션·세트 메뉴) 구축"),
    ("RTOS 연동 및 출력 처리", "영수증/주문번호표 출력 요청(print_jobs) 프론트 연동, RTOS 폴링·완료 처리 흐름을 검증하는 백엔드 통합 테스트 작성"),
]
...
summary_rows = [
    ...
    (2, 24, [(1, "담당자: 박유진 | 기간: 2026-07-13 ~ 2026-08-25", 2)]),
    (4, 24, [(1, "구분", 3), (2, "내용", 3)]),
    (5, 24, [(1, "총 커밋", 4), (2, "99건", 4)]),
    (6, 24, [(1, "작업 기간", 4), (2, "2026-07-13 ~ 2026-08-25 (44일)", 4)]),
...
summary_rows.append((16, 65, [(1, "요약: 고객용 키오스크의 프론트엔드 전 화면(홈·메뉴·장바구니·결제·주문완료)을 담당하고, 네이버페이·카카오페이·토스페이 결제 연동, 세트 메뉴·품절 로직, 다국어·반응형 대응, 자동 테스트 및 Playwright E2E 테스트를 수행했으며, 영수증/주문번호표 출력(print_jobs) 연동과 이를 검증하는 백엔드 통합 테스트도 수행함.", 5)]))
...
<body>
  ...
  <div class="subtitle">담당자: 박유진 · 기간: 2026-07-13 ~ 2026-08-25</div>
  <div class="summary">
    <div class="card"><strong>총 커밋</strong>99건</div>
    <div class="card"><strong>작업 기간</strong>44일</div>
  ...
  <div class="note"><b>업무 요약</b><br>고객용 키오스크의 프론트엔드 전 화면(홈·메뉴·장바구니·결제·주문완료)을 담당하고, 네이버페이·카카오페이·토스페이 결제 연동, 세트 메뉴·품절 로직, 다국어·반응형 대응, 자동 테스트 및 Playwright E2E 테스트를 수행했으며, 영수증/주문번호표 출력(print_jobs) 연동과 이를 검증하는 백엔드 통합 테스트도 수행함.</div>
