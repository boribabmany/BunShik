...
2026-08-27 기준 최신 실행 결과:

- 고객용: 13개 테스트 스위트, 62개 테스트 통과
- 관리자용: 22개 테스트 스위트, 84개 테스트 통과
- 프론트 합계: 35개 테스트 스위트, 146개 테스트 통과
- Playwright E2E: 고객 주문(매장·포장·옵션·세트) 4개, 관리자(인증·메뉴·주문 상태·다중 처리·주문번호 검색 결과 없음) 7개 시나리오 통과
- 백엔드: 23개 테스트 스위트, 112개 테스트 통과(고객 주문·결제, 관리자 인증·주문·환불, print_jobs 통합 포함)
- 별도 `BunShik_RTOS` 저장소: CTest 12개 통과(2026-08-27 재실행). 관리자 주문·카탈로그 감시와 고객 출력 Worker, 프린터 오류·완료 요청 재시도·복구 흐름을 검증함.
- DevProject Hub QA: 87건 모두 PASS (2026-08-27 조회 기준)
